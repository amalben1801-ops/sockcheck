...
class AppTests(unittest.TestCase):
    ...
    def test_login_and_assets(self):
        with app.test_client() as visitor:
            ...
            self.assertIn(b'3800', response.data)
            self.assertEqual(visitor.get('/detector').status_code, 200)
        response = self.client.get('/detector')
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'name="sock_a"', response.data)
        self.assertIn(b'name="sock_b"', response.data)
        self.assertIn(b'app.js?v=4', response.data)
        self.assertIn(b'not-same.mp4', response.data)
        for path in ['/static/app.js', '/static/style.css', '/static/extras.css', '/static/favicon.svg', '/static/login-hero.jpg', '/static/not-same.mp4']:
